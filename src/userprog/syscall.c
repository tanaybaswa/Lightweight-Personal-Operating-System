...
#define INVALID_STACK -1
  ...
  for (i = 0; i < bytes; i++) {
    if (!esp8)
      break;
    if (!is_user_vaddr(esp8))
      break;
    uint32_t* pte = lookup_page(pd_base, esp8, false);
    if (!pte)
      break; /* No page table entry existed. Bad. */
   ...
   */
    ...
    if (((pte_flags & PTE_P) != PTE_P) || ((pte_flags & PTE_U) != PTE_U)) {
      break;
    }
    if (!allow_rw) {
      if ((pte_flags & PTE_W) == PTE_W) {
        break;
      }
    }
